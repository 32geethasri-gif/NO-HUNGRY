...
function App() {
  ...
  const handleMyDonations = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/food/my-donations?donor=${loggedInUser.id}`
      );
  ...
  const handleRequestFood = async (foodId) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/food-request/request",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleMyRequests = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/food-request/all"
      );
...
{loggedInUser?.role === "donor" && (
  <section id="my-donations">
    ...
    <p>
      View the food you have donated through NO HUNGER.
    </p>
...
{loggedInUser?.role === "admin" && (
  <section id="admin-dashboard">
    ...
    <p>
      Welcome to the NO HUNGER administration area.
    </p>
